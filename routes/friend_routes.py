...
def send_request():
    ...
    # ... (كود التحقق والتعامل مع حالات الـ 400 و 404) ...

    # ملاحظة: هذا المسار غالباً هو سبب الـ 422 إذا كان user_id أو friend_username مفقوداً.

    # ... (بقية الكود الخاص بإنشاء الطلب والـ commit) ...

    return jsonify({'message': 'Friend request sent', 'status': 'pending'})


# --- 6. جلب جميع المستخدمين للبحث (GET /friends/users) ---
# تم تعديله ليقبل user_id لتجنب عرض المستخدم الحالي في قائمة البحث
...
def get_all_users_for_search(current_user_id):
    ...
    for user in users_query:
        # يمكنك إضافة نقاط المستخدم إذا كانت موجودة في نموذج User
